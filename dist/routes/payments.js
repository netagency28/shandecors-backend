...
const parseBool = (value) => cleanEnv(value).toLowerCase() === 'true';
const resolveGateway = () => {
    ...
    if (explicit === 'cashfree' || explicit === 'instamojo')
        return explicit;
    if (instamojoEnabled && !cashfreeEnabled)
        return 'instamojo';
    return 'cashfree';
};
const getCashfreeConfig = () => {
    ...
    if (!enabled) {
        return { ok: false, message: 'Cashfree gateway is disabled via ENABLE_CASHFREE=false.' };
    }
...
const getInstamojoConfig = () => {
    ...
    if (!enabled) {
        return { ok: false, message: 'Instamojo gateway is disabled via ENABLE_INSTAMOJO=false.' };
    }
    if (!clientId || !clientSecret) {
        return {
            ok: false,
...
const getInstamojoAccessToken = async (config) => {
    const form = new URLSearchParams();
    form.set('grant_type', 'client_credentials');
    form.set('client_id', config.clientId);
    form.set('client_secret', config.clientSecret);
    const tokenUrls = [config.tokenUrl, config.tokenUrlFallback].filter(Boolean);
    let lastError = null;
...
router.post('/create-order', async (req, res) => {
    try {
        ...
        const apiBases = [config.apiBaseUrl, config.apiBaseUrlFallback].filter(Boolean);
        let imJson = {};
        let success = false;
        let lastCreateError = '';
        for (const apiBase of apiBases) {
            ...
            try {
                ...
                imJson = (await imResponse.json().catch(() => ({})));
                if (imResponse.ok) {
                    success = true;
                    break;
                }
...
router.get('/verify/:orderId', async (req, res) => {
    try {
        ...
        if (!order)
            return res.status(404).json({ message: 'Order not found' });
        const gateway = String(req.query.gateway || order.paymentMethod || resolveGateway()).toLowerCase();
        if (gateway === 'cashfree') {
            const config = getCashfreeConfig();
            if (!config.ok) {
                return res.status(500).json({
